...
export function CountdownMenu() {
	const { isFinished, isActive, startCountdown, resetCountdown, skipTimer } = useContext(CountdownContext)
	...
	return (
			<div role={"menu"} aria-label="Timer actions" className='flex justify-between w-full items-center px-4'>
				...
				<button type='button' role={"menuitem"} onClick={openModal} aria-label="Get break exercise" className={'group relative z-20 btn btn-circle items-center overflow-hidden hover:btn-secondary '+(isFinished ? ' pulse btn-accent shadow-accent/30' : ' btn-ghost text-accent')}>
				...
				<div className="text-secondary-content bg-gradient-to-b from-secondary to-secondary-focus h-20 w-20 rounded-full border-secondary border-4 shadow-lg shadow-accent/20 flex items-center justify-center">
					...
					<>
					{ isActive ? (
						<button type='button' onClick={resetCountdown} aria-label="Reset countdown" className="btn btn-ghost hover:btn-secondary btn-circle btn-lg flex justify-center items-center relative z-20">
						...
						<button type='button' onClick={startCountdown} aria-label="Start countdown" className="btn btn-ghost hover:btn-secondary btn-circle btn-lg flex justify-center items-center group relative z-20">
				...
				{ isActive ? (
					<button type='button' onClick={skipTimer} aria-label="Skip timer" className="btn btn-ghost hover:btn-secondary btn-circle flex justify-center items-center relative z-20">
						<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 pointer-events-none">
					...
					aria-label='Cycle ended'  
					disabled
					className="btn btn-circle btn-disabled flex justify-center items-center">
						<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 pointer-events-none">
							<path strokeLinecap="round" strokeLinejoin="round" d="M3 8.688c0-.864.933-1.405 1.683-.977l7.108 4.062a1.125 1.125 0 010 1.953l-7.108 4.062A1.125 1.125 0 013 16.81V8.688zM12.75 8.688c0-.864.933-1.405 1.683-.977l7.108 4.062a1.125 1.125 0 010 1.953l-7.108 4.062a1.125 1.125 0 01-1.683-.977V8.688z" />
						</svg>
					</button>
