...
const Tabs = ({ router }) => {
	...
	return (
		...
		<nav className="flex justify-center py-2 gap-8 bg-accent/80 rounded-t-2xl">
			<div role={"tablist"} className="flex justify-center py-2 gap-8">
			...
			<Link href={{ pathname: "/", query: { tab: "tasks" } }} className={"flex flex-col justify-center items-center rounded-2xl flex-grow-0 p-2 hover:opacity-60 transition-opacity text-accent-content "+(isTaskTab ? "bg-secondary/10 shadow-md shadow-accent-content/20" : "")}>
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6" aria-hidden="true">
